...
function validateCurrentRows() {
  ...
  $(".product-list-row .product-item-row").each(function () {
    ...
    var totalquantity = $(this).find(".product-quantity-available").html();
    ...
    success: function (response) {
      if (response.status == '1') {
        ...
        if (data) {
          ...
          $(targetparent).find(".rate").val(data.purchase_price);
          $(targetparent).find(".purchase_price_with_tax").val(data.purchase_price_with_tax);
...
function UpdateCalculations() {
  ...
  $(".product-list-row .product-item-row").each(function () {
    ...
    if (quantity != "" && rate != "") {
      ...
      purchase_price_with_tax = $(this).find('.purchase_price_with_tax').val();
    
      purchase_row_total = purchase_price_with_tax * quantity/(1+gst/100);

      row_total = Math.round(purchase_row_total * gst_decimal_rounding) / gst_decimal_rounding;

      $(this).find(".total_without_tax").val(1234);
      ...
      row_total_amount = Math.round(row_total * taxable_decimal_rounding) / taxable_decimal_rounding;
      grand_total_value += row_total;
      row_total_total += row_total;

      $(this).find(".line_total").val(row_total_amount);
